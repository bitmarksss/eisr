...
class StockController extends Controller
{
    ...
    public function receivingStore(Request $request)
    {
        ...
        $itemSupplierMatches = InventoryItem::query()
            ->whereIn('id', collect($data['items'])->pluck('item_name'))
            ->pluck('supplier_id', 'id');

        foreach ($data['items'] as $index => $item) {
            if ((int) ($itemSupplierMatches[$item['item_name']] ?? 0) !== (int) $item['supplier_id']) {
                return back()->withErrors(["items.$index.item_name" => 'The selected item does not belong to the selected supplier.'])->withInput();
            }
        }
        ...
        DB::transaction(function () use ($data) {
            $movement = StockMovement::create([
                ...
                'notes' => json_encode([
                    ...
                    'supplier_ids' => collect($data['items'])->pluck('supplier_id')->unique()->values()->all(),
        ...
        $movements = StockMovement::with([
            'user', 'level', 'items.item.kind', 'items.item.unit', 'items.item.supplier', 'approvals.user'])
            ->where('type', 'receive')
            ->latest('movement_date')
            ->latest('id')
            ->paginate(15);
    ...
    {
        $requestFormItems = InventoryItem::with(['kind', 'unit'])
            ->get()
            ...
            ->values();
        $requestFormSuppliers = Supplier::orderBy('name')
            ->get(['id', 'name']);

        return view('pages.stock.requests.form', compact('requestFormItems', 'requestFormSuppliers'));
    }
