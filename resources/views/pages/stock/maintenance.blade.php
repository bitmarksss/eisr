...
<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center justify-between w-full">
            <!-- Search and Filters -->
            <form method="GET" action="{{ route($location . '.stock.index', ['location' => $location]) }}" class="flex flex-wrap items-center gap-3 flex-1 w-full">
                <div class="flex w-90">
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-6 py-4 font-bold">
                                <span class="text-brand-dark">
                                    {{ number_format($stock->quantity['quantity'] ?? 0) }} {{ $stock->uom->name ?? '' }}
                                </span>
                            </td>

                            <!-- User Column -->
                            <td class="px-6 py-4 text-gray-500">
                                {{ $stock->user->name ?? 'Unknown' }}
                            </td>
                            ...
                            @if(auth()->user()?->role_id == 1 || auth()->user()?->is_admin)
                                <td class="px-6 py-4 text-right whitespace-nowrap">
                                    <button onclick="openEditStockRequestModal('{{ $stock->id }}', '{{ addslashes($stock->type) }}', '{{ $stock->kind }}', '{{ $stock->quantity['quantity'] ?? 0 }}', '{{ $stock->status }}')" 
