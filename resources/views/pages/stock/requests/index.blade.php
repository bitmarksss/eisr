            ...
            @forelse($stockRequests as $stockRequest)
                <tr class="transition hover:bg-gray-50/50">
                    ...
                    <td class="px-6 py-4 text-right space-x-2">
                        ...
                        >
                            <button type="button" class="view-request cursor-pointer rounded-lg bg-brand-navy px-2.5 py-2 text-xs font-bold text-white hover:underline" data-request='@json($stockRequest)'>
                        ...
                        <x-tooltip 
                            ...
                            text_color="text-white"
                        >
                            <!-- <button type="button" class=" cursor-pointer rounded-lg bg-brand-green px-2.5 py-2 text-xs font-bold text-white hover:underline" data-request='@json($stockRequest)'>
                                <i class="fa-solid fa-print"></i>
                            </button> -->
                            <a class="cursor-pointer rounded-lg bg-brand-green px-2.5 py-2 text-xs font-bold text-white
    ...
    const render = request => {
        const rows = (request.items || []).map(line => { 
            ...
            <tr class="border-t border-gray-100">
                <td class="px-4 py-3 text-center font-bold text-brand-navy">${esc(line.quantity)}</td>
                <td class="px-4 py-3 text-gray-600">${esc(item.kind?.kind || '—')}</td>
                <td class="px-4 py-3 font-semibold text-brand-dark">${esc([item.name, item.variant].filter(Boolean).join(' ') || '—')}</td>
                <td class="px-4 py-3 text-gray-600">${esc(item.supplier?.name || '—')}</td>
                <td class="px-4 py-3 text-gray-600">${esc(item.unit?.unit || '—')}</td>
        ...
        const base = `
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                    ...
                    <p class="mt-1 font-semibold text-brand-dark">${esc(value)}</p>
                </div>`).join('')}
            </div>
            <section>
                <div class="mb-2 flex items-center justify-between">
                    <h4 class="text-sm font-bold uppercase tracking-wider text-brand-navy">Requested items</h4>
                    <span class="text-xs text-gray-500">${(request.items || []).length} item(s)</span>
                </div>
                <div class="overflow-x-auto rounded-xl border border-gray-100">
                    <table class="w-full min-w-[760px] text-left text-sm">
                        ...
                        <tbody>
                            ${rows || 
                            `<tr>
                                <td colspan="6" class="px-4 py-5 text-center text-gray-500">No requested items found.</td>
                            </tr>`}
                        </tbody>
                    </table>
                </div>
